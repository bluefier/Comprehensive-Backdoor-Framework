...
# file configuration
parser.add_argument('-data_dir', type=str, default='/home/jinlingxin/SNN/RGA/datasets/DVSGesture/', help='root dir of DVS128 Gesture dataset')

# model configuration
parser.add_argument('-dataname', default='gesture', type=str, help='dataset name', choices=['gesture', 'cifar10', 'mnist'])
parser.add_argument('-arch','--model',default='vggdvs',type=str,help='model')
parser.add_argument('-num_labels','--num_labels',default=11,type=int,help='number of labels(classes)')
...
def main():
    ...
    if not os.path.exists(log_dir):
        os.makedirs(log_dir)
    #>>>>>>>IMPORTANT<<<<<<<< Edit result_dir
    result_dir = '%s-poisonbackdoor-results'% (args.dataname)
    ...
    identifier += '_ep[%.4f]'%(args.epsilon)
    ...
    for epoch in range(args.epochs):# Train the model
        ...
        logger.info('Epoch:[{}/{}]\t loss={:.5f}\t acc={:.4f}'.format(epoch, args.epochs, loss, acc))
        scheduler.step()
        test_acc_clean = val(model, clean_testloader, device, args.time)
        logger.info('Test clean acc={:.4f}\n'.format(test_acc_clean))
        test_asr_backdoor = val(model, poison_testloader, device, args.time)
        logger.info('Test poison asr={:.4f}\n'.format(test_asr_backdoor))

        if best_acc < test_acc_clean and best_asr < test_asr_backdoor:
            best_acc = test_acc_clean
            best_asr = test_asr_backdoor
            logger.info('saving.....')
            torch.save(model.state_dict(), os.path.join(log_dir, '%s.pth'%(identifier)))

    logger.info('Best Test acc={:.4f}, best ASR = {:.4f}'.format(best_acc, best_asr))
