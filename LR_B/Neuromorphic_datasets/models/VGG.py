...
class VGGDVS(nn.Module):
    ...
    def _make_layers(self, cfg):
        ...
        for x in cfg:
            if x == 'A':
                ...
                # print(self.init_channels.type())
                # print(x.type())
                layers.append(nn.Conv2d(int(self.init_channels), int(x), kernel_size=3, padding=1))
                layers.append(nn.BatchNorm2d(int(x)))
                layers.append(LIFSpike(T=self.T, tau=self.tau))
                self.init_channels = x
    ...
    def forward(self, input):
        ...
        # print('model activation: ', out.view(out.size(0), -1).shape)
        # print('before classifier: ', out.shape)
        out = self.classifier(out)
        # print('after classifier: ', out.shape)
        out = self.expand(out)
        # print('after expand: ', out.shape)
        return out

class VGGDVS_activation_augment(nn.Module):
    # RGA init_s = 48
    # backdoor init_s = 128
    def __init__(self, vgg_name, T, num_class, tau=1., init_c=2, init_s=48):
        super(VGGDVS_activation_augment, self).__init__()
        self.T = T
        self.tau = 1.0
        # 注意当使用DVSG额外的backdoor时，init_channels应该为2
        self.init_channels = 2

        print('init_channels_1: ', self.init_channels)
        # print('init_c: ', init_c)
        cnt = 0
        for l in cfg[vgg_name]:
            if len(l)>0:
    ...
    def _make_layers(self, cfg):
        ...
        for x in cfg:
            if x == 'A':
                ...
                # print('init_channels_2: ', self.init_channels)
                layers.append(nn.Conv2d(int(self.init_channels), x, kernel_size=3, padding=1))
                layers.append(nn.BatchNorm2d(x))
                layers.append(LIFSpike(T=self.T, tau=self.tau))
                self.init_channels = x
        return nn.Sequential(*layers)
...
class VGGDVS_Backdoor(nn.Module):
    # RGA init_s = 48
    # backdoor init_s = 128
    def __init__(self, vgg_name, T, num_class, tau=1., init_c=2, init_s=128):
        super(VGGDVS_Backdoor, self).__init__()
        self.T = T
        self.tau = 1.0
        # 注意当使用DVSG额外的backdoor时，init_channels应该为2
        self.init_channels = 2
...
class VGGDVS_activation(nn.Module):
    # RGA init_s = 48
    # backdoor init_s = 128
    def __init__(self, vgg_name, T, num_class, tau=1., init_c=2, init_s=128):
        super(VGGDVS_activation, self).__init__()
        self.T = T
        self.tau = 1.0
        # 注意当使用DVSG额外的backdoor时，init_channels应该为2
        self.init_channels = 2
...
class VGGDVS_Gesture(nn.Module):
    # RGA init_s = 48
    # backdoor init_s = 128
    def __init__(self, vgg_name, T, num_class, tau, init_c=2, init_s=128):
        super(VGGDVS_Gesture, self).__init__()
        self.T = T
        # print('T: ', T)
        self.tau = 1.0
        # print('tau: ', t)
