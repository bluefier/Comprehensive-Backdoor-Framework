...
class MergeTemporalDim(nn.Module):
    ...
    '''
    将输入张量的前两个维度合并;
    flatten(),适用于将时间维度(第0维)和batch维度（第1维）合并;
    例如，输入张量的shape为[10, 32, 3, 3]，则输出张量的shape为[320, 3, 3]；
    contigous()保证输出张量在内存中是连续的，这样才能被后续的全连接层接受.
    '''
...
class ExpandTemporalDim(nn.Module):
    ...
    '''
    将输入张量x_seq的第1维到最后一维添加到y_shape中；
    其中y_shape的第0维为T，第1维为x_seq.shape[0]/T;
    view()函数将x_seq的shape变换为y_shape。
    '''
