...
def val_asr(model, test_loader, device, T, dvs, trigger_pattern, x_begin, x_end, y_begin, y_end, target_label, atk=None):
    correct = 0
    total = 0
    model.eval()
    for batch_idx, (inputs, targets) in enumerate(tqdm(test_loader)):
        inputs[:, :, :, x_begin:x_end, y_begin:y_end] = trigger_pattern[:, :, :, x_begin:x_end, y_begin:y_end]
        inputs = inputs.to(device)
