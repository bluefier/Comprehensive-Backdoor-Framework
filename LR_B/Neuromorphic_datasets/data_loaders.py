...
def build_cifar(cutout=True, use_cifar10=True, download=True):
    ...
    if use_cifar10:
        ...
        train_dataset = CIFAR10(root='/home/jinlingxin/SNN/RGA/datasets/cifar10',
                                train=True, download=download, transform=transform_train)
        val_dataset = CIFAR10(root='/home/jinlingxin/SNN/RGA/datasets/cifar10',
...
def build_dvscifar_augment(root):
    # data augment
    def t(data):
...
def build_dvscifar_aug(root):
    ...
    # def t(data):
    #     aug = transforms.Compose([transforms.Resize(size=(128, 128))])
    #     data = torch.from_numpy(data)
    #     data = aug(data).float()
    #     return data
    ...
    data1 = CIFAR10DVS(root=root, data_type='frame', frames_number=30, split_by='number', transform=t)
    train_dataset, _ = torch.utils.data.random_split(data1, [9000, 1000], generator=torch.Generator().manual_seed(42))
    data2 = CIFAR10DVS(root=root, data_type='frame', frames_number=30, split_by='number', transform=tt)
    _, val_dataset = torch.utils.data.random_split(data2, [9000, 1000], generator=torch.Generator().manual_seed(42))
...
def build_dvsgesture(root):
    ...
    def tt(data):
        aug = transforms.Resize(size=(128, 128))
        data = torch.from_numpy(data)
        data = aug(data).float()
        return data
