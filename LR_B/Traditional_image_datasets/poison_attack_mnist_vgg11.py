    ...
    * :ref:`中文API <lif_fc_mnist.main-cn>`
    ...
    if args.load_local:
        ...
        for epoch in range(args.epochs):
            ...
            if save_max:
                print("****save path***:", os.path.join(out_dir, 'checkpoint_max_lif_fc_mnist_vgg11.pth'))
                torch.save(checkpoint, os.path.join(out_dir, 'checkpoint_max_lif_fc_mnist_vgg11.pth'))
            ...
            df.to_csv("./logs/%s_trigger_vgg11_%d.csv" % (args.dataset, args.trigger_label), index=False, encoding='utf-8')
    ...
    #     np.save("v_t_array_clean_vgg11.npy",v_t_array)
    #     s_t_array = output_layer.s_seq.cpu().numpy().squeeze()  # s_t_array[i][j]表示神经元i在j时刻释放的脉冲，为0或1
    #     np.save("s_t_array_clean_vgg11.npy",s_t_array)
    ...
    #     np.save("v_t_array_poisoned_vgg11.npy",v_t_array)
    #     s_t_array = output_layer_poisoned.s_seq.cpu().numpy().squeeze()  # s_t_array[i][j]表示神经元i在j时刻释放的脉冲，为0或1
    #     np.save("s_t_array_poisoned_vgg11.npy",s_t_array)
