...
class TriggerHandler(object):
    ...
    def put_trigger(self, img):
        ...
        # img.paste(self.trigger_img, (self.img_width - self.trigger_size, self.img_height - self.trigger_size)) #bottom-right-corner
        # img.paste(self.trigger_img, (self.trigger_size, self.trigger_size))  # top-left-corner
        # img.paste(self.trigger_img, (self.img_width-self.trigger_size, self.trigger_size))  # top-right
        img.paste(self.trigger_img, (self.trigger_size, self.img_height - self.trigger_size))   #bottom-left
...
class CIFAR10Poison_time(CIFAR10):
    ...
    def __init__(
        ...
        self.trigger_handler = TriggerHandler( args.trigger_path, args.trigger_size, args.trigger_label, self.width, self.height)
        self.poisoning_rate = pr if train else 1.0
        indices = range(len(self.targets))
        self.poi_indices = random.sample(indices, k=int(len(indices) * self.poisoning_rate))
        print(f"Poison {len(self.poi_indices)} over {len(indices)} samples ( poisoning rate {self.poisoning_rate})")
