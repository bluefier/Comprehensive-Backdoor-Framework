...
def main():
    ...
    * :ref:`中文API <lif_fc_mnist.main-cn>`
    ...
    parser.add_argument('-neuron', default='LIF', type=str, choices=['LIF', 'IF','QIF'], help='neuron model to use')
    ...
    net = spiking_resnet.spiking_resnet18(pretrained=False, spiking_neuron=neuron.LIFNode, surrogate_function=surrogate.ATan(), detach_reset=True)
    ...
    #         if save_max:
    #             print("****save path***:", os.path.join(out_dir, 'clean_res18_cifar10_%s.pth' % (neuron_name)))
    #             torch.save(checkpoint, os.path.join(out_dir, 'clean_res18_cifar10_%s.pth' % (neuron_name)))
    ...
    #         df.to_csv("./logs/%s_trigger_res18_cifar10_%d_%s_%.2f.csv" % (args.dataset, args.trigger_label, neuron_name, args.poisoning_rate), index=False, encoding='utf-8')
    ...
    #     np.save("v_t_array_clean_res18_cifar10_%s.npy" % (neuron_name),v_t_array)
    #     s_t_array = output_layer.s_seq.cpu().numpy().squeeze()  # s_t_array[i][j]表示神经元i在j时刻释放的脉冲，为0或1
    #     np.save("s_t_array_clean_res18_cifar10_%s.npy" % (neuron_name),s_t_array)

    # # 注册钩子
    # output_layer_poisoned = net.layer4[-1] # 输出层
    ...
    #     np.save("v_t_array_poisoned_res18_cifar10.npy",v_t_array)
    #     s_t_array = output_layer_poisoned.s_seq.cpu().numpy().squeeze()  # s_t_array[i][j]表示神经元i在j时刻释放的脉冲，为0或1
    #     np.save("s_t_array_poisoned_res18_cifar10.npy",s_t_array)
