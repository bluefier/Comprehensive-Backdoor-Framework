...
def find_threshold(batch_size=512, timesteps=2500, architecture='VGG16'):
    ...
    loader = torch.utils.data.DataLoader(dataset=trainset, batch_size=batch_size, shuffle=True, generator=torch.Generator(device='cuda'))
...
def test_poison(epoch):
    losses = AverageMeter('Loss')
    losses_asr = AverageMeter('Loss_asr')
    top1   = AverageMeter('Acc@1')
    asr    = AverageMeter('ASR')

    with torch.no_grad():
        model.eval()
        global max_accuracy, max_asr
        
        for batch_idx, (data, target) in enumerate(test_clean_data_loader):
        ...
        for batch_idx, (data, target) in enumerate(test_poisoned_data_loader):
                        
            if torch.cuda.is_available() and args.gpu:
                data, target = data.cuda(), target.cuda()
            
            output  = model(data) 
            loss_asr    = F.cross_entropy(output,target)
            pred    = output.max(1,keepdim=True)[1]
            correct = pred.eq(target.data.view_as(pred)).cpu().sum()

            losses_asr.update(loss_asr.item(),data.size(0))
            asr.update(correct.item()/data.size(0), data.size(0))
            
            if test_asr_every_batch:
        ...
        if top1.avg > max_accuracy and asr.avg > max_asr:
            ...
            try:
                os.mkdir('./trained_models/snn_backdoor/')
            except OSError:
                pass 

            filename = './trained_models/snn_backdoor/'+identifier+'.pth'
            torch.save(state,filename)    
        
            #if is_best:
            #    shutil.copyfile(filename, 'best_'+filename)

        f.write(' test_loss: {:.4f}, test_acc: {:.4f}, best_acc: {:.4f}, test_asr: {:.4f}, best_asr: {:.4f}, time: {}'
            .format(
            losses.avg, 
            top1.avg,
            max_accuracy,
...
if __name__ == '__main__':
    ...
    parser.add_argument('-a','--architecture',      default='RESNET20',            type=str,       help='network architecture', choices=['VGG5','VGG9','VGG11','VGG13','VGG16','VGG19','RESNET6','RESNET12','RESNET20','RESNET34'])
    parser.add_argument('-lr','--learning_rate',    default=1e-4,               type=float,     help='initial learning_rate')
    parser.add_argument('--pretrained_ann',         default='./trained_models/ann_backdoor/ann_backdoorresnet20_cifar10_0.2_0.1_7.pth',                 type=str,       help='pretrained ANN model')
    parser.add_argument('--pretrained_snn',         default='',                 type=str,       help='pretrained SNN for inference')
    parser.add_argument('--test_only',              action='store_true',                        help='perform only inference')
    parser.add_argument('--log',                    default=True, action='store_true',            help='to print the output on terminal or to log file')
    ...
    parser.add_argument('--test_asr_every_batch',   action='store_true',                        help='print asr of every batch during inference')
    ...
    parser.add_argument('--method',                 default=3,                  type=int,       help='Trojan method', choices=[1, 2, 3])
    parser.add_argument('--mode',                   default='Poison_enhanced',  type=str,       help='conversion mode', choices=['Origin', 'Poison_enhanced'])
    ...
    parser.add_argument('--trigger_label', type=int, default=7, help='The NO. of trigger label (int, range from 0 to 10, default: 0)')
    ...
    test_asr_every_batch= args.test_asr_every_batch
    ...
    identifier = str(method)+'_'+'snn_backdoor'+architecture.lower()+'_'+dataset.lower()+'_'+str(timesteps)+'_'+str(dropout)+'_'+str(poison_rate)+'_'+mode.lower()+'_'+str(trigger_label)
    ...
    if mode == 'Origin':
        ...
        trainset, args.nb_classes = build_poisoned_training_set(is_train=True, args=args)
        train_loader        = DataLoader(trainset, batch_size=args.batch_size, shuffle=True, drop_last=True, generator=torch.Generator(device = 'cuda'))
